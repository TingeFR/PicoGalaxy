...
import { updateNotionDate, updateNotionRows, updateNotionToDos } from "./notionUpdater";

export const menuMaker = async (menusRepository: Repository<Menu>, mealsRepository: Repository<Meal>) => {

  // ---- PREPARATIFS ----
  
  const menus = await menusRepository.find({ relations: [ "meals" ] });
  const meals = await mealsRepository.find({ relations: [ "ingredients", "steps" ] });
  ...
  let coursesList: {quantity: number, unit: string, name: string}[] = []
  newMeals.forEach(meal => {
    meal.ingredients.forEach(ingredient => {
      const index = coursesList.findIndex(item => item.name === ingredient.name)
      if(index != -1){
